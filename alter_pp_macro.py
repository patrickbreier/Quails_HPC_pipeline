...
with open('/projects/Quails/scripts/pipeline/preprocessing_template.ijm', 'r') as file:
    data = file.read().replace('image_number', image_number).replace('in_dir', in_dir).replace('input_directory', in_dir).replace('rb_radius', rb_radius).replace('out_dir', out_dir).replace('in_file', in_name).replace('block_size', block_size)
